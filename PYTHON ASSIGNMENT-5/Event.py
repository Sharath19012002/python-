...
from DBConnUtil import DbConn


class Event(DbConn):
    def __init__(self, event_id=None, event_name=None, event_date=None, event_time=None, venue_id=None,
                 total_seats=None, availble_seats=None, ticket_price=None, event_type=None):
        ...
        self.available_seats = availble_seats
    ...
    def calculate_total_revenue(self):
        try:
            self.open()
            self.c.execute(
                f"Select ticket_price*(total_seats - available_seats) From Event Where event_id = {self.event_id}")
    ...
    def book_tickets(self, num_tickets):
        try:
            if num_tickets <= self.available_seats:
                self.open()
                self.c.execute(
                    f"Update Event Set available_seats = ({self.available_seats} - {num_tickets}) Where event_id = {self.event_id}")
                self.mydb.commit()
                self.close()
                return True
            else:
