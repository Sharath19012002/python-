...
class TicketBookingSystem(DbConn):
    ...
    def create_event(self, event_name, date, time, venue_id, total_seats, available_seats, ticket_price, event_type):
        try:
            self.open()
            self.c.execute(
                f"Insert Into Event (event_name, event_date, event_time, venue_id, total_seats, available_seats, ticket_price, event_type) Values ('{event_name}', '{date}', '{time}', {venue_id}, {total_seats}, {available_seats}, {ticket_price}, '{event_type}')")
            self.mydb.commit()
            self.close()
